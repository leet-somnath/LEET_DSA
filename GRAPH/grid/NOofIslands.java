    ...
    private void bfs(int ro, int co, int[][] vis, char[][] grid) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                // Check if the neighboring cell is valid, unvisited, and is land ('1')
    ...
    public int numIslands(char[][] grid) {
        ...
        for (int row = 0; row < n; row++) {
            for (int col = 0; col < m; col++) {
                // If the current cell is unvisited and is land ('1')
                if (vis[row][col] == 0 && grid[row][col] == '1') {
                    cnt++; 
                    bfs(row, col, vis, grid); // Start BFS from this cell
                }
            }
        }
        return cnt; 
